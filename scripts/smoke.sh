...
export ADAPTER_DISTILL=off
harness_start smoke smoke-key

ADD_A='{
  "request_id": "req-001", "user_id": "user-a", "session_id": "sess-1",
  "messages": [
    {"role": "user", "content": "The build for the parser project uses make lint before every commit", "timestamp": 1786615200000},
...
R=$(post add "$ADD_A")
check "identical Add retry is idempotent" 'HTTPSTATUS:200' "$R"

R=$(post add '{
  "request_id": "req-001", "user_id": "user-a", "session_id": "sess-1",
  "messages": [{"role": "user", "content": "different source payload"}]}')
check "request id reuse with different source conflicts" 'HTTPSTATUS:409' "$R"
...
R=$($CURL -o /dev/null -w '%{http_code}' -X POST "http://$ADDR/search" -H 'Content-Type: application/json' -d '{"query":"x","user_id":"u","top_k":5}')
